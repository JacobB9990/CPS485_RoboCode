...
WEIGHTS_PATH="$ROOT_DIR/$WEIGHTS_FILE"
LOG_DIR="$ROOT_DIR/logs/test_$(date +%Y%m%d_%H%M%S)"
...
if [ ! -f "$WEIGHTS_PATH" ]; then
  ...
  ls -lh "$ROOT_DIR"/*.pt 2>/dev/null | awk '{print "  " $NF}' || echo "  (none)"
  exit 1
fi
...
DQN_MODE="eval" \
  ...
  "$ROOT_DIR/headless_runner/run_battle.sh" "$DQN_BOT_DIR" "$OPPONENT_DIR" "$ROUNDS" 0 \
