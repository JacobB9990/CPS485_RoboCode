#!/usr/bin/env bash
# One-command overnight trainer for Jacob3_0 DQN bot.
#
# What it does:
...
#   WEIGHTS_NAME=my.pt       Weights file name in Jacob3_0 (default: dqn_weights_curriculum.pt)

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
RUN_BATTLE_CMD="$ROOT_DIR/headless_runner/run_battle.sh"
DQN_BOT_DIR="$ROOT_DIR/HeadlessDQNBot"
LOG_ROOT="$ROOT_DIR/logs/overnight_$(date +%Y%m%d_%H%M%S)"
PASSES="${PASSES:-2}"
...
WEIGHTS_PATH="$ROOT_DIR/$WEIGHTS_NAME"
...
if [ ! -f "$ROOT_DIR/tools/robocode-tankroyale-runner-0.38.2.jar" ]; then
  echo "Missing runner jar. Download with:"
  echo "  curl -L --fail -o \"$ROOT_DIR/tools/robocode-tankroyale-runner-0.38.2.jar\" https://github.com/robocode-dev/tank-royale/releases/download/v0.38.2/robocode-tankroyale-runner-0.38.2.jar"
  exit 1
fi
