...
SCRIPT_NAME="train_ppoadvanced_curriculum"
...
write_jsonl_header "$SCRIPT_NAME" "PPOBotAdvanced"

CHECKPOINT_DIR="$ROOT_DIR/PPOBotAdvanced/checkpoints"

# ── Helpers ───────────────────────────────────────────────────────────────────

# Win-rate failsafe: halt if last 50 episodes in a stage have <5% win rate.
# SpinBot canary: near-stationary bot — anything below 40% after 50 eps means aim/fire is broken.
check_win_rate() {
...
try:
    ...
    with open(jsonl_path) as f:
        for line in f:
            ...
            try:
                r = json.loads(line)
                sc = r.get("scenario", "")
                if sc.startswith(scenario_base) and r.get("record_type") != "summary":
                    rows.append(r)
            except Exception:
                pass
    last50 = rows[-50:]
    if len(last50) < 10:
        sys.exit(0)
    win_rate = sum(1 for r in last50 if r.get("won")) / len(last50)
    print(f"[failsafe] {scenario_base}: win_rate={win_rate:.1%} over last {len(last50)} eps")
    if scenario_base == "ppoadvanced_vs_spinbot" and len(last50) >= 50 and win_rate < 0.40:
        print(f"FAILSAFE: SpinBot canary: win_rate={win_rate:.1%} < 40% — aim/fire bug suspected.")
        sys.exit(1)
    elif win_rate < 0.05:
        print(f"FAILSAFE: win_rate={win_rate:.1%} < 5% — halting curriculum.")
        sys.exit(1)
...
# Worker 1 = writer (PPOBotAdvanced), workers 2..N = readers (PPOBotAdvancedReader, stale weights).
...
run_stage() {
    ...
    for i in $(seq 1 "$PARALLEL"); do
        ...
        tmp_jsons+=("$tmp_json")
        local bot_key="PPOBotAdvancedReader"
        [[ "$i" -eq 1 ]] && bot_key="PPOBotAdvanced"
...
if [[ "$SMOKE_TEST" -eq 1 ]]; then
    ...
    # 1. Import check
    cd "$ROOT_DIR"
    python3 -c "from PPOBotAdvanced.runtime.PPO_Bot import PPOBot; print('Import OK')"
...
run_stage  1  "ppoadvanced_vs_target"      "$(stage_r 300)"  Target
run_stage  2  "ppoadvanced_vs_fire"        "$(stage_r 200)"  Fire
run_stage  3  "ppoadvanced_vs_velocitybot" "$(stage_r 200)"  VelocityBot
run_stage  4  "ppoadvanced_vs_walls"       "$(stage_r 200)"  Walls
run_stage  5  "ppoadvanced_vs_spinbot"     "$(stage_r 200)"  SpinBot
run_stage  6  "ppoadvanced_vs_corners"     "$(stage_r 200)"  Corners
run_stage  7  "ppoadvanced_vs_crazy"       "$(stage_r 200)"  Crazy
run_stage  8  "ppoadvanced_vs_trackfire"   "$(stage_r 200)"  TrackFire
run_stage  9  "ppoadvanced_vs_ramfire"     "$(stage_r 200)"  RamFire
run_stage  R1 "ppoadvanced_refresher_1"   "$(stage_r  50)"  Target Fire
run_stage 10  "ppoadvanced_3bot_easy"      "$(stage_r 200)"  Target Fire
run_stage 11  "ppoadvanced_3bot_medium"    "$(stage_r 200)"  SpinBot Walls
run_stage 12  "ppoadvanced_4bot_hard"      "$(stage_r 150)"  Crazy TrackFire RamFire
run_stage  R2 "ppoadvanced_refresher_2"   "$(stage_r  50)"  Target Fire
run_stage 13  "ppoadvanced_5bot_mix"       "$(stage_r 150)"  SpinBot Crazy TrackFire RamFire
run_stage 14  "ppoadvanced_vs_jacob3_0"    "$(stage_r 200)"  Jacob3_0
