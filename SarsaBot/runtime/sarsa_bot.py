...
SARSA_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_Q_TABLE_PATH = SARSA_ROOT / "data" / "q_table_sarsa.json"
DEFAULT_LOG_PATH = SARSA_ROOT / "logs" / "training_log.jsonl"
...
class SarsaBot(Bot):
	...
	def _select_action(self, state: str) -> int:
		epsilon = self.eval_epsilon if self.eval_mode else self.epsilon
	...
	def on_bot_death(self, bot_death_event) -> None:  # noqa: ANN001
		name = getattr(bot_death_event, "name", None)
		victim_name = getattr(bot_death_event, "victim_name", None)
