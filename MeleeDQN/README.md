...
Python conversion of the melee DQN Robocode Tank Royale bot family.

## Layout

- `runtime/` holds the converted bot runtimes.
- `agent/` holds the DQN and SARSA learners.
- `training/` holds the socket AI server used by the DQN bot.
- `config/` contains Tank Royale bot descriptors.
- `checkpoints/` stores learned weights.
- `logs/` stores runtime or training logs.
...
./HybridMeleeBot.sh
./MeleeSarsaBot.sh
./MeleeOpponentModelBot.sh
```

The DQN bot attempts to connect to a local socket AI server at `127.0.0.1:5000`. If the server is unavailable, it falls back to local DQN action selection.

## Run the socket trainer

```bash
python3 -m MeleeDQN.training.train_melee_dqn_socket --host 127.0.0.1 --port 5000
```

## Notes

- The converted melee state uses the same 48-feature encoding as the Java version.
- The action set preserves the 15-action Java enum order.
- The runtime includes the translated enemy tracking, danger map, target selection, reward shaping, and terminal reward handling.
- The hybrid bot keeps the rule-based tactical ladder, target scoring, and danger-map navigation.
- The SARSA bot persists its Q-table to `data/q_table_sarsa.json`.
