...
from Jacob3_0.agent.dqn_agent import DQNAgent
...
DEFAULT_WEIGHTS_PATH = DQN_ROOT / "checkpoints" / "dqn_weights.pt"
DEFAULT_LOG_PATH = DQN_ROOT / "logs" / "dqn_training_log.jsonl"
DEFAULT_STATE_LOG_PATH = DQN_ROOT / "logs" / "game_states.jsonl"
...
class DQNBot(Bot):
    ...
    def on_bullet_hit(self, bullet_hit_bot_event) -> None:
        ...
        self.last_damaged_enemy_name = getattr(bullet_hit_bot_event, "name", None)
    ...
    def on_bot_death(self, bot_death_event: BotDeathEvent) -> None:
        if getattr(bot_death_event, "victim_name", None) == self.last_damaged_enemy_name:
            if (self.local_tick - self.last_damage_tick) <= 2:
                self.kills += 1
        elif getattr(bot_death_event, "name", None) == self.last_damaged_enemy_name:
            if (self.local_tick - self.last_damage_tick) <= 2:
                self.kills += 1
    ...
    def _append_state_snapshot(
        ...
        if self.last_scan is not None:
            enemies.append(
                {
                    "x": round(self.last_scan.x, 3),
                    "y": round(self.last_scan.y, 3),
                    "energy": round(self.last_scan.energy, 3),
                    "age_ticks": self.local_tick - self.last_scan.turn,
                }
            )
        ...
        try:
            ...
            print(f"[DQNBot] State log failed: {exc}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--state-log-path",
        default="",
        help="Optional JSONL path for per-tick state collection.",
    )
    parser.add_argument(
        "--eval",
        action="store_true",
